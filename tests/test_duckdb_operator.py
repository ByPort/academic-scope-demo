...
from common.operators.duckdb import DuckDBOperator, DuckDBDecoratedOperator, duckdb_task
...
class TestDuckDBOperator:
    ...
    def test_execute_success(self, mock_hook_class, mock_airflow_connections):
        ...
        # Assertions
        mock_hook_class.assert_called_once_with(duckdb_conn_id='duckdb_default', config=None)
    ...
    def test_execute_with_config(self, mock_hook_class, mock_airflow_connections):
        ...
        # Assertions
        mock_hook_class.assert_called_once_with(duckdb_conn_id='custom_conn', config=config)
        mock_hook.sql.assert_called_once_with('CREATE TABLE test (id INT)')
        assert result == mock_result
    
    @patch('common.operators.duckdb.DuckDBHook')
    def test_execute_exception_handling(self, mock_hook_class, mock_airflow_connections):
...
class TestDuckDBDecoratedOperator:
    ...
    @patch('common.operators.duckdb.DuckDBHook')
    def test_decorated_operator_execute(self, mock_hook_class, mock_airflow_connections):
