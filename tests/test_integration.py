"""Integration tests for the arxiv_etl DAG.

These tests verify real DAG behavior with minimal mocking to catch actual bugs.
"""
import pytest
...
class TestDAGStructure:
    ...
    def test_task_dependencies(self, dagbag):
        ...
        prepare_warehouse = dag.get_task('prepare_warehouse')
        ...
        extract_upstream = [t for t in extract_pubs.upstream_task_ids]
        load_upstream = [t for t in load_pubs.upstream_task_ids]
        
        assert 'prepare_warehouse' in extract_upstream
        assert 'extract_publications' in load_upstream
        
        # Check branching structure
        downstream_task_ids = [t for t in check_task.downstream_task_ids]
...
class TestBranchingLogic:
    ...
    def test_check_if_zip_exists_when_file_exists(self, mock_exists, mock_connection_get, dagbag):
        ...
        result = task.python_callable(choices=("skip_download_zip", "prepare_raw_folder"))
    ...
    @patch('airflow.sdk.definitions.connection.Connection.get')
    @patch('pathlib.Path.exists')
    def test_check_if_zip_exists_when_file_missing(self, mock_exists, mock_connection_get, dagbag):
...
class TestSQLTemplateValidation:
    ...
    def test_extract_publications_sql_syntax(self, dagbag):
        """Test that extract_publications returns valid SQL with correct connection references."""
        dag = dagbag.get_dag(dag_id="arxiv_etl")
...
class TestDuckDBTaskExecution:
    ...
    def test_prepare_warehouse_executes_correct_sql(self, mock_get_connection, dagbag):
        ...
        mock_get_connection.return_value = MockConnection('duckdb_default', {'database': ':memory:'})
...
class TestErrorScenarios:
    ...
    def test_missing_connection_raises_error(self, mock_connection_get, dagbag):
        ...
        mock_connection_get.side_effect = KeyError("Connection 'missing_conn' not found")
