...
class TestDuckDBHook:
    ...
    def test_sql_execution(self, mock_duckdb_connect, mock_airflow_connections):
        ...
        mock_duckdb_connect.return_value.__enter__ = MagicMock(return_value=mock_conn_instance)
    ...
    @patch('duckdb.connect')
    def test_sql_execution_with_config(self, mock_duckdb_connect, mock_airflow_connections):
    ...
    @patch('duckdb.connect')
    def test_sql_execution_exception_handling(self, mock_duckdb_connect, mock_airflow_connections):
    ...
    def test_hook_logging(self, mock_airflow_connections, caplog):
        ...
        assert "Initializing DuckDBHook with connection ID duckdb_default" in caplog.text
